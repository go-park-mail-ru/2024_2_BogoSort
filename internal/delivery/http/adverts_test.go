...
func NewTestSessionManager() *utils.SessionManager {
	sessionManager := utils.NewSessionManager(nil, 0, false, nil)
...
func setupAdvertEndpoints(t *testing.T) (*AdvertEndpoints, *mocks.MockAdvertUseCase, *mocks.MockStaticUseCase, *utils.SessionManager, *zap.Logger) {
	...
	endpoints := NewAdvertEndpoints(mockAdvertUseCase, mockStaticUseCase, sessionManager, logger)
...
func TestAdvertEndpoints_GetAdverts(t *testing.T) {
...
func TestAdvertEndpoints_GetAdvertsBySellerId(t *testing.T) {
	endpoints, mockAdvertUseCase, _, _, _ := setupAdvertEndpoints(t)
	defer endpoints.logger.Sync()
...
func TestAdvertEndpoints_GetAdvertsByCartId(t *testing.T) {
	endpoints, mockAdvertUseCase, _, _, _ := setupAdvertEndpoints(t)
	defer endpoints.logger.Sync()
...
func TestAdvertEndpoints_GetAdvertById(t *testing.T) {
	endpoints, mockAdvertUseCase, _, _, _ := setupAdvertEndpoints(t)
	defer endpoints.logger.Sync()
...
func TestAdvertEndpoints_UpdateAdvert(t *testing.T) {
	endpoints, _, _, _, _ := setupAdvertEndpoints(t)
	defer endpoints.logger.Sync()
...
func TestAdvertEndpoints_DeleteAdvertById(t *testing.T) {
	endpoints, _, _, _, _ := setupAdvertEndpoints(t)
	defer endpoints.logger.Sync()
...
func TestAdvertEndpoints_UpdateAdvertStatus(t *testing.T) {
	endpoints, _, _, _, _ := setupAdvertEndpoints(t)
	defer endpoints.logger.Sync()
	...
	t.Run("Invalid status", func(t *testing.T) {
		...
		req := httptest.NewRequest("PUT", "/api/v1/adverts/"+advertID.String()+"/status", bytes.NewBufferString("status="+invalidStatus))
		req = mux.SetURLVars(req, map[string]string{
			"advertId": advertID.String(),
		})
		rr := httptest.NewRecorder()
		...
		if statusCode := rr.Code; statusCode != http.StatusBadRequest {
			t.Errorf("Expected status %v, got %v", http.StatusBadRequest, statusCode)
		}
...
func TestAdvertEndpoints_GetAdvertsByCategoryId(t *testing.T) {
	endpoints, mockAdvertUseCase, _, _, _ := setupAdvertEndpoints(t)
	defer endpoints.logger.Sync()
...
func TestAdvertEndpoints_UploadImage(t *testing.T) {
	endpoints, _, _, _, _ := setupAdvertEndpoints(t)
	defer endpoints.logger.Sync()
